...
|                             Nested JSON fields `field$=json` | Useful when sending complex JSON. For example `wow[such][deep][3][much][power][!]$=Amaze` or `highlander[]$=one`. It supports the [JSON form](https://www.w3.org/TR/html-json-forms/) syntax.                          |
